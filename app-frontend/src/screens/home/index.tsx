...
interface GeneratedResult {
  expression: string;
  answer: string;
  steps: Array<string>; // Add steps to the result interface
}
...
interface Response {
  ...
  result: string;
  assign: boolean;
  steps: Array<string>; // Add steps to the response interface
}
...
export default function Home() {
  ...
  const [isEraser, setIsEraser] = useState(false);
  const [showAnalysis, setShowAnalysis] = useState(false);
  const [steps, setSteps] = useState<Array<string>>([]); // State for step-by-step breakdown
  ...
  useEffect(() => {
    if (result) {
      renderLatexToCanvas(result.expression, result.answer, result.steps);
    }
  }, [result]);
  ...
  const renderLatexToCanvas = (
    ...
    const latex = `${expression} = ${answer}`;
    setLatexExpression([...latexExpression, latex]);
    setSteps(steps); // Set the step-by-step explanations in state
  };
    ...
    if (canvas) {
      ...
      resp.data.forEach((data: Response) => {
        ...
        setResult({
          expression: data.expr,
          answer: data.result,
          steps: data.steps, // Extract steps from response
        });
      });
  ...
  return (
    <>
      <div className="w-full flex items-center justify-center bg-black h-[80px]">
        <div className="text-bold text-3xl text-white logo">
          <span className="text-[aqua]">BrainWave</span> <span className="ps-3">Solver</span>{' '}
        </div>
      </div>
      <div className="grid grid-cols-4 gap-2">
        <Button
      ...
      {/* <Draggable position={latexPosition}>
        <div className="latex-container z-10">
          {latexExpression.map((expression, index) => (
            <div key={index} dangerouslySetInnerHTML={{ __html: `$${expression}$` }} />
          ))}
        </div>
      </Draggable> */}
      <div className="fixed top-[120px] left-[400px] z-0">
        <canvas
          ref={canvasRef}
          onMouseDown={startDrawing}
          onMouseMove={draw}
      ...
      {showAnalysis && steps.length > 0 && (
        <div className="steps-container bg-white text-black p-4">
